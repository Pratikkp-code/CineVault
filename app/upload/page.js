...
import { UploadCloud, Film, ImageIcon, Video, DollarSign, CheckCircle, AlertCircle, Loader2, PartyPopper, Link as LinkIcon } from "lucide-react"
  ...
  return (
    <div className="flex items-center gap-2 mt-2 text-sm">
      ...
      <p className={getStatusColor()}>
        <span className="font-semibold truncate max-w-[200px] inline-block align-middle">{file.name}</span>
...
export default function UploadPage() {
  ...
  const [uploadStatus, setUploadStatus] = useState("idle") // idle, uploading, uploaded, error, submitting
  ...
  const uploadSingleFile = async (type, file) => {
    try {
      ...
      const cid = await uploadFileToIPFS(file, { useOriginFirst: true, metadata: { fileType: type, movieTitle: formData.title, uploadedBy: address } })
      setUploadedFiles((prev) => ({ ...prev, [type]: { cid, status: "completed" } }))
      toast({ title: "Upload Success", description: `${type.charAt(0).toUpperCase() + type.slice(1)} uploaded successfully!` })
      return cid
    } catch (error) {
  ...
  const uploadFilesToIPFS = async () => {
    ...
    setUploadProgress(0)
    try {
      const filesToUpload = Object.entries(files).filter(([, file]) => file).map(([key, file]) => ({ key, file }))
      const results = {}
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!isConnected || (userProfile && !userProfile.exists) || !files.film || !formData.title.trim() || !formData.price48h || parseFloat(formData.price48h) <= 0) {
      toast({ title: "Validation Error", description: "Please fill all required fields and connect your wallet.", variant: "destructive" })
      return
    }
    try {
      setUploadStatus("submitting")
      const uploadResults = await uploadFilesToIPFS()
      const uploadFee = platformStats ? platformStats[2] : parseEther("0.01")
      await writeContract({
        address: CONTRACT_ADDRESS,
        abi: CONTRACT_ABI,
        functionName: "uploadMovie",
        args: [formData.title, formData.description || "", formData.genre || "", formData.language || "", uploadResults.film || "", uploadResults.trailer || "", uploadResults.thumbnail || "", parseEther(formData.price48h)],
        value: uploadFee,
      })
    } catch (error) {
  ...
  const uploadFeeCAMP = useMemo(() => (platformStats ? (Number(platformStats[2]) / 1e18).toFixed(4) : "0.01"), [platformStats])
  const isUploadingOrConfirming = uploadStatus === "uploading" || uploadStatus === "submitting" || isPending || isConfirming
          ...
          <Card className="bg-gray-800 border-gray-700 max-w-lg text-center shadow-2xl shadow-teal-500/10">
            ...
            <CardContent className="space-y-4">
              <div className="bg-gray-900 rounded-lg p-3 text-left space-y-2">
                {uploadedFiles.film.cid && <p className="text-xs text-gray-400 break-all"><strong className="text-teal-400">Film CID:</strong> {uploadedFiles.film.cid}</p>}
                {uploadedFiles.trailer.cid && <p className="text-xs text-gray-400 break-all"><strong className="text-cyan-400">Trailer CID:</strong> {uploadedFiles.trailer.cid}</p>}
                {uploadedFiles.thumbnail.cid && <p className="text-xs text-gray-400 break-all"><strong className="text-pink-400">Thumbnail CID:</strong> {uploadedFiles.thumbnail.cid}</p>}
              </div>
              <p className="text-sm text-gray-500">Transaction Hash: <a href={`https://sepolia.etherscan.io/tx/${hash}`} target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:underline break-all">{hash}</a></p>
            </CardContent>
            <CardFooter className="flex-col md:flex-row gap-2">
              ...
              <Link href="/profile" className="w-full">
                <Button variant="outline" className="w-full border-cyan-400 text-cyan-400 hover:bg-cyan-400 hover:text-white font-bold">
                  View My Profile
                </Button>
              </Link>
            </CardFooter>
      ...
      <motion.div className="max-w-4xl mx-auto px-4" initial="hidden" animate="visible" variants={itemVariants}>
        ...
        {isConnected && userProfile && !userProfile.exists && (
          <Alert variant="destructive" className="mb-6 bg-yellow-900/50 border-yellow-700 text-yellow-300">
            ...
            <AlertDescription>You need to create a profile before uploading. <Link href="/profile" className="font-bold underline hover:text-yellow-200">Create Profile</Link></AlertDescription>
          </Alert>
        )}
        ...
        <Card className="bg-gray-800/80 backdrop-blur-sm border-gray-700 shadow-lg">
          <form onSubmit={handleSubmit}>
            <CardContent className="p-6 md:p-8 space-y-8">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <Label htmlFor="title">Movie Title <span className="text-red-500">*</span></Label>
                    <Input id="title" name="title" value={formData.title} onChange={handleInputChange} required disabled={isUploadingOrConfirming} placeholder="e.g., The Decentralized Dream" />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="price48h">48h Rental Price (CAMP) <span className="text-red-500">*</span></Label>
                    <Input id="price48h" name="price48h" type="number" step="0.0001" min="0.0001" value={formData.price48h} onChange={handleInputChange} required disabled={isUploadingOrConfirming} placeholder="e.g., 0.01" />
                  </div>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="description">Description</Label>
                  <Textarea id="description" name="description" value={formData.description} onChange={handleInputChange} rows={4} disabled={isUploadingOrConfirming} placeholder="A short synopsis of your movie..." />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <Label>Genre</Label>
                    <Select value={formData.genre} onValueChange={(v) => setFormData(p => ({ ...p, genre: v }))} disabled={isUploadingOrConfirming}>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={formData.language} onValueChange={(v) => setFormData(p => ({ ...p, language: v }))} disabled={isUploadingOrConfirming}>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <div className="space-y-2">
                    <Label className="flex items-center gap-2"><Video className="h-4 w-4" />Movie File <span className="text-red-500">*</span></Label>
                    <Input type="file" accept="video/*" onChange={(e) => handleFileChange("film", e.target.files[0])} disabled={isUploadingOrConfirming} />
              ...
              <div className="space-y-6">
                {uploadStatus === "uploading" && (
                  <div className="space-y-2">
                    <div className="flex justify-between font-semibold"><span>Uploading to IPFS...</span><span>{Math.round(uploadProgress)}%</span></div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <Alert className="bg-gray-900/70 border-gray-700">
                    ...
                    <AlertDescription className="text-xs text-gray-400 space-y-1 mt-2">
                      <p>72h Price: {formData.price48h ? (parseFloat(formData.price48h) * 1.4).toFixed(4) : "0"} CAMP</p>
                      <p>1 Week Price: {formData.price48h ? (parseFloat(formData.price48h) * 2.8).toFixed(4) : "0"} CAMP</p>
                  ...
                  <Alert className="bg-gray-900/70 border-gray-700">
                    ...
                    <AlertDescription className="text-gray-400 mt-2">
                      A one-time fee of <strong className="text-cyan-300">{uploadFeeCAMP} CAMP</strong> is required to upload your movie.
                    </AlertDescription>
                  </Alert>
                </div>
              </div>
            ...
            <CardFooter className="bg-gray-800 p-6">
              <Button type="submit" disabled={!isConnected || isUploadingOrConfirming || (userProfile && !userProfile.exists)} className="w-full bg-teal-500 hover:bg-teal-600 font-bold text-lg py-6 disabled:bg-gray-600 disabled:cursor-not-allowed">
                {
                  !isConnected ? "Connect Wallet to Upload" :
                  userProfile && !userProfile.exists ? "Please Create a Profile First" :
                  isUploadingOrConfirming ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" /> <span>{isConfirming ? "Processing Transaction..." : isPending ? "Confirming in Wallet..." : "Uploading Files..."}</span></> :
                  canSubmit ? <> <CheckCircle className="mr-2 h-5 w-5" /> Ready to Submit to Blockchain</> :
