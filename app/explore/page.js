...
export default function ExplorePage() {
    ...
    const { data: allMovies, isLoading } = useReadContract({
    ...
    useEffect(() => {
        ...
        switch (sortBy) {
            ...
            case 'popular':
                ...
                break
            case 'price-low':
                filtered.sort((a, b) => Number(a.price48h) - Number(b.price48h))
                break
            case 'price-high':
                filtered.sort((a, b) => Number(b.price48h) - Number(a.price48h))
                break
        }
                                ...
                                <Card className="bg-gray-800 border-gray-700 hover:border-teal-500 transition-colors duration-300 overflow-hidden group h-full flex flex-col">
                                    <CardHeader className="p-0">
                                        <div className="aspect-video overflow-hidden">
                                            <motion.img
                                                src={movie.thumbnailCID ? `https://gateway.pinata.cloud/ipfs/${movie.thumbnailCID}` : `/placeholder.svg?height=225&width=400&query=${encodeURIComponent(movie.title + ' movie poster')}`}
                                                alt={movie.title}
